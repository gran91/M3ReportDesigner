...
public class Blowfish
{
    ...
    private void Blowfish_encipher(long xx[])
    {
        int Xl = (int)(xx[0] >> 32);
        int Xr = (int)xx[0];
        int temp;
        ...
        Xr ^= P[N];
        Xl ^= P[N + 1];
        xx[0] = ((long)Xl & 0xffffffffL) << 32 | (long)Xr & 0xffffffffL;
    }

    private void InitializeBlowfish(StringBuffer keyS, int keybytes)
    {
        ...
        {
            Blowfish_encipher(dataxx);
            datal[0] = (int)(dataxx[0] >> 32);
            datar[0] = (int)dataxx[0];
            P[i] = datal[0];
            P[i + 1] = datar[0];
        }
        ...
        for(int i = 0; i < 4; i++)
            for(j = 0; j < 256; j += 2)
            {
                dataxx[0] = ((long)datal[0] & 0xffffffffL) << 32 | (long)datar[0] & 0xffffffffL;
                Blowfish_encipher(dataxx);
                datal[0] = (int)(dataxx[0] >> 32);
                datar[0] = (int)dataxx[0];
                S[i][j] = datal[0];
    ...
    private void blf_key(StringBuffer k, int len)
    {
        InitializeBlowfish(k, len);
    }

    public long byteArrayToLong(byte b[], int startIndex)
    {
        return ((long)b[startIndex] & 255L) << 56 | ((long)b[startIndex + 1] & 255L) << 48 | ((long)b[startIndex + 2] & 255L) << 40 | ((long)b[startIndex + 3] & 255L) << 32 | ((long)b[startIndex + 4] & 255L) << 24 | ((long)b[startIndex + 5] & 255L) << 16 | ((long)b[startIndex + 6] & 255L) << 8 | (long)b[startIndex + 7] & 255L;
    }

    public void longToByteArray(long l, byte b[], int startIndex)
    {
        b[startIndex] = (byte)(int)(l >> 56 & 255L);
        b[startIndex + 1] = (byte)(int)(l >> 48 & 255L);
        b[startIndex + 2] = (byte)(int)(l >> 40 & 255L);
        b[startIndex + 3] = (byte)(int)(l >> 32 & 255L);
        b[startIndex + 4] = (byte)(int)(l >> 24 & 255L);
        b[startIndex + 5] = (byte)(int)(l >> 16 & 255L);
        b[startIndex + 6] = (byte)(int)(l >> 8 & 255L);
        b[startIndex + 7] = (byte)(int)(l & 255L);
    }

    private int byteArrayToLongArray(byte b[], long l[], int byteLen)
    {
        byte bTmp[] = new byte[8];
        int longLen = byteLen / 8;
        int i;
        for(i = 0; i < longLen; i++)
        {
            for(int j = 0; j < 8; j++)
            ...
            l[i] = byteArrayToLong(bTmp, 0);
        }

        return i;
    }

    private int longArrayToByteArray(long l[], byte b[], int longLen)
    {
        byte bTmp[] = new byte[8];
        int i;
        for(i = 0; i < longLen; i++)
        {
            longToByteArray(l[i], bTmp, 0);
            for(int j = 0; j < 8; j++)
                b[i * 8 + j] = bTmp[j];
    ...
    private long[] blf_cfb_enc(long data[], int blocks)
    {
        byte bufferC[] = new byte[8];
        byte cipherBlock[] = new byte[8];
        byte dataBlock[] = new byte[8];
        long cipher[] = new long[blocks];
        long bufferL[] = new long[1];
        bufferL[0] = iv;
        for(int i = 0; i < blocks; i++)
        {
            longToByteArray(data[i], dataBlock, 0);
            for(int j = 0; j < 8; j++)
            {
                Blowfish_encipher(bufferL);
                longToByteArray(bufferL[0], bufferC, 0);
                cipherBlock[j] = (byte)(bufferC[0] ^ dataBlock[j]);
                for(int k = 0; k < 7; k++)
                    bufferC[k] = bufferC[k + 1];

                bufferC[7] = cipherBlock[j];
            }

            cipher[i] = byteArrayToLong(cipherBlock, 0);
        }

        return cipher;
    }

    private long[] blf_cfb_dec(long data[], int blocks)
    {
        byte bufferC[] = new byte[8];
        byte plainBlock[] = new byte[8];
        byte dataBlock[] = new byte[8];
        long plain[] = new long[blocks];
        long bufferL[] = new long[1];
        bufferL[0] = iv;
        for(int i = 0; i < blocks; i++)
        {
            ...
            for(int j = 0; j < 8; j++)
            {
                Blowfish_encipher(bufferL);
                longToByteArray(bufferL[0], bufferC, 0);
                plainBlock[j] = (byte)(bufferC[0] ^ dataBlock[j]);
                for(int k = 0; k < 7; k++)
                    bufferC[k] = bufferC[k + 1];

                bufferC[7] = dataBlock[j];
            }

            plain[i] = byteArrayToLong(plainBlock, 0);
        }

        return plain;
    }

    private int align(StringBuffer inStr, int inLen)
    {
        char at = '@';
        int r;
        if((r = inLen % 8) != 0)
    ...
    int dealign(StringBuffer inStr, int inLen)
    {
        for(int i = inLen; inStr.charAt(i - 1) == '@'; i--)
            inStr.setLength(inStr.length() - 1);

        return inStr.length();
    }

    public int BF_cfb64_encrypt(StringBuffer inStr, StringBuffer outStr, int inLen, StringBuffer key, int keyLen, long iv)
    {
        long inLong[] = new long[inLen / 8 + 1];
        long outLong[] = new long[inLen / 8 + 1];
        byte tmpStr[] = new byte[inLen + 8];
        blf_key(key, keyLen);
        int tmp;
        if((tmp = align(inStr, inLen)) > 0)
            inLen = tmp;
        for(int i = 0; i < inLen; i++)
            tmpStr[i] = (byte)inStr.charAt(i);

        byteArrayToLongArray(tmpStr, inLong, inLen);
        outLong = blf_cfb_enc(inLong, inLen / 8);
        int outLen = longArrayToByteArray(outLong, tmpStr, inLen / 8);
        for(int i = 0; i < outLen; i++)
            outStr.insert(i, (char)tmpStr[i]);

        return outLen;
    }

    public int BF_cfb64_decrypt(StringBuffer inStr, StringBuffer outStr, int inLen, StringBuffer key, int keyLen, long iv)
    {
        long inLong[] = new long[inLen / 8 + 1];
        long outLong[] = new long[inLen / 8 + 1];
        byte tmpStr[] = new byte[inLen + 8];
        ...
        byteArrayToLongArray(tmpStr, inLong, inLen);
        outLong = blf_cfb_dec(inLong, inLen / 8);
        int outLen = longArrayToByteArray(outLong, tmpStr, inLen / 8);
        for(int i = 0; i < outLen; i++)
            outStr.insert(i, (char)tmpStr[i]);

        outLen = dealign(outStr, outLen);
        return outLen;
    }

    public static void main(String args[])
    {
        StringBuffer key = new StringBuffer("AAAAA");
        StringBuffer inStr = new StringBuffer("1234567890abcdefghijklmnopqrstuvwxyz");
        ...
        System.out.println("in : [" + inStr + "] [" + inStr.length() + "]");
        int i = bf.BF_cfb64_encrypt(inStr, bufStr, inStr.length(), key, 5, bf.iv);
        System.out.println("buf: [" + bufStr + "] [" + i + "]");
        int j = bf.BF_cfb64_decrypt(bufStr, outStr, i, key, 5, bf.iv);
        System.out.println("out: [" + outStr + "] [" + j + "]");
    }
